...
void run_regression_checks() {
    ...
    double expected_ratio = (y2 * y2) / (y1 * y1);
    ...
    double sign_check = (v_bottom * v_top < 0) ? 1 : 0;
    
    if (sign_check && fabs(magnitude_ratio - 1.0) < 0.2) {
    ...
    double y_near_bottom = W * 0.25;
    double y_near_top = W * 0.75;
    ...
    int dir_check_bottom = (v_near_bottom > 0) ? 1 : 0;
    int dir_check_top = (v_near_top < 0) ? 1 : 0;
