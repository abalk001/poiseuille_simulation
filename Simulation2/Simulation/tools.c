...
// Improved lift coefficient based on lambda and nu
// For lambda=1, U ≈ 0.15 + 0.3*(1-nu)/(1-0.6) from paper fit
// For other lambda values, includes lambda dependence
double lift_velocity_coefficient(double lambda, double nu) {
    if (nu >= 1.0) return 0.0;
    if (nu <= 0.0) return 0.5; // Highly deflated limit
    
    // Paper-based empirical fit for U(lambda, nu)
    // U increases with deflation (decreasing nu) and with lambda
    double nu_factor = (1.0 - nu) / (1.0 - 0.6); // Normalized deflation
    double lambda_factor = 0.5 * (1.0 + lambda) / (1.0 + lambda); // Viscosity ratio effect
    
    // Base coefficient tuned to experimental data
...
double migration_velocity_poiseuille(double y, double W, double U_max, 
    ...
    // Use nearest-wall distance for symmetric treatment
    double d_bottom = y - vesicle_r0;           // Distance from bottom wall
    double d_top = (W - y) - vesicle_r0;        // Distance from top wall
    double d_nearest = fmin(d_bottom, d_top);   // Nearest wall gap
    
    // Smooth regularization: cap minimum distance to avoid singularity
    double d_min = 0.1 * vesicle_r0;  // Regularization parameter
    double d_reg = fmax(d_nearest, d_min);
    
    // Migration velocity formula with regularized distance
    ...
    // Direction: based on which wall is closer (symmetric lift)
    // If closer to bottom wall, move up; if closer to top wall, move down
    if (d_bottom < d_top) {
        return v_magnitude;   // Move away from bottom wall (upward)
    } else {
        return -v_magnitude;  // Move away from top wall (downward)
    }
}
...
// Run simulation with specified parameters using RK4 integration
void run_simulation(const char* mode, const char* output_file) {
    ...
    // Open file for streaming output (memory efficient)
    ...
    // Lambda function for computing velocity at position y_eval
    ...
    for (int i = 0; i < steps; i++) {
        ...
        // Stream output at specified intervals (memory efficient)
        ...
        // Stability safeguard: limit maximum displacement per step
        double max_dy = 0.1 * vesicle_r0;  // Max 10% of radius per step
        if (fabs(dy) > max_dy) {
...
int validate_parameters(double W, double r0, double nu, double U_max, double dt) {
    ...
    if (dt <= 0.0) {
        ...
        // Stability warning for dt
        double max_velocity_estimate = U_max * 0.5;  // Rough estimate
